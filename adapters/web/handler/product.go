...
func MakeProductHandlers(router *mux.Router, n *negroni.Negroni, service application.ProductServiceInterface) {
	...
	router.Handle("/product/{id}/enable", n.With(negroni.Wrap(enableProduct(service)))).Methods("OPTIONS", "POST")
	router.Handle("/product/{id}/disable", n.With(negroni.Wrap(disableProduct(service)))).Methods("OPTIONS", "POST")
}
